  ...
  try {
    ...
    if (oldPassword === "123456" && ismatched) {
      if (newPassword === "123456") {
        return res
          .json({ msg: " Please change this default password" });
      }
      if (newPassword.length < 5) {
        return res
          
          .json({ msg: "Password must be at least 5 chars" });
      }
...
app.get('/viewNotifications', auth, async (req, res) => {
  try {
    const existingUser = await members.findOne({ id: req.id }).select(['notifications', '-_id'])
    if (!existingUser) {
...
//12,2020
    ...
    if (!monthly) {
      return res
        .json({ msg: "Couldn't find the record Not yet calculated" });
    }
  ...
  try {
    ...
    if (!ismatched) {
      return res
        ...
        .json({ msg: "Your Current Password doesn't match ur password" });
    }
    ...
    if (newPassword === "123456") {
      return res
        ...
        .json({ msg: "You aren't allowed to go back to the default password" });
    }
...
app.get("/signin", auth, async (req, res) => {
  try {
    ...
    let min = `${moment().hour() < 7
      ? "00"
      : moment().minute() < 10
        ? "0" + moment().minute()
        : moment().minute()
      }`;
    let day = `${moment().date() < 10 ? "0" + moment().date() : moment().date()}`;
    //.month() function returns the month-1

    let month = `${(moment().month() + 1) < 10 ? "0" + (moment().month() + 1) : (moment().month() + 1)}`;
  ...
  try {
    ...
    if (signinlist.length === 0) {
      return res
        ...
        .json({msg:"You can't signout because u didn't sign in "});
    }
    ...
    var attendedmonthstring=attendmonth<10?"0"+attendmonth:attendmonth
...
app.put("/faculties/:name", auth, async (req, res) => {
  try {
    ...
    if (repeatedFac) {
      return res
        .json({ msg: "Cannot use this name as it already exists in system" });
    }
...
app.get('/viewDepartments', auth, async (req, res) => {
  try {
    ...
    const existingDepartment = await departments.find()
    .populate({ path: "faculty", select: { _id: 0, name: 1 } })
  ...
  try {
    ...
    if (head) {
      return res
        ...
        .json({ msg: "You can only assign head through update member's role" });
    }
...
app.get('/viewCourses', auth, async (req, res) => {
  try {
    ...
    const existingCourse = await courses.find()
...
app.post("/addcourse", auth, async (req, res) => {
  try {
    ...
    if (coordinator) {
      return res
        .json({ msg: "Only course instructors can add coordinator" });
    }
    if (instructors) {
      return res
        .json({ msg: "HR cannot assign course members, instructors" });
    }
    if (teachingassistants) {
      return res
        .json({ msg: "HR cannot assign course members,TAs" });
    }
    if (slots) {
      return res
...
app.get('/viewmember/:id', auth, async (req, res) => {
  try {
    ...
    const member = await members.findOne({id:req.params.id}).populate({ path: "officelocation", select: { _id: 0, locname: 1 } })
    .populate({ path: "department", select: { _id: 0, name: 1 } })
    .populate({ path: "faculty", select: { _id: 0, name: 1 } });
...
app.post("/addmember", auth, async (req, res) => {
  try {
    ...
    if (role == "HOD" && !department) {
      return res
        .json({ msg: "Please enter a department for member to be head" });
    }
    if (!email) {
...
app.put("/members/missesdays", auth, async (req, res) => {
  try {
    ...
    if (!monthly) {
      return res
        .json
        ({msg:"Couldn't find the record Not yet calculated"});
    }
...
app.put("/updatesignout/:id", auth, async (req, res) => {
  ...
  try {
    ...
    const mawgood = await members.findOne({ id: req.id });
    if (!mawgood) {
      return res.json( {msg: "This User(token) making the request doesn't Exist in my Database"})
    ...
    //.month() function returns the month-1
    let signoutmin = `${hour>=19?"00":min >= 0 && min < 10 ? "0" + min : min}`;
    let signoutday = `${day < 10 ? "0" + day : day}`;
    //.month() function returns the month-1
    let signoutmonth = `${month < 10 ? "0" + month : month}`;
    let signoutyear = `${year}`;
    let dayindex=moment(`${signoutyear}-${signoutmonth}-${signoutday}T${signouthour}:${signoutmin}`
    ).day();
    ...
    if (signinlist.length === signoutlist.length || signinlist.length === 0) {
      return res
        .json({msg:"You can't manually add a signout with out signing in ?? "});
    }
...
app.put("/updatemember/:id", auth, async (req, res) => {
  try {
    ...
    let { name, role, office, faculty, department, bio, gender,salary } = req.body;
    if (role == "HOD" && !department) {
...
app.delete("/removeinstructorfromcourse", auth, async (req, res) => {
  ...
  try {
    ...
    if (roleuser != "Course Instructor" && instructor != req.id) {
      return res
        .json({ msg: "the id must belong to course instructor" });
    }
    const instDep = existinguser.department;
    if (!instDep.equals(hodDep)) {
      return res
        .json({ msg: "Instructor does not belong to your department" });
    }
    ...
    if (foundcourse) {
      ...
      if (!foundin) {
        return res
          .json({ msg: "the instructor is not assigned to the course " });
      } else {
  ...
  try {
    ...
    if (oldCourse) {
      ...
      if (newCourse) {
        ...
        if (!oldcourseInDep) {
          return res
            .json({ msg: "Old Course is not in your deparment" });
        }
        const newcourseInDep = newCourse.departments.includes(hodDep);
        if (!newcourseInDep) {
          return res
            .json({ msg: "New Course is not in your deparment" });
        }
        ...
        if (!foundin) {
          return res
            .json({ msg: "the instructor is not assigned to the old course " });
        } else {
...
app.put("/viewDayoff", auth, async (req, res) => {
  try {
    ...
    if (!staffid) {
      ...
      if (!s) {
        return res
          .json({ msg: "Staff id not found in department" });
      }
      const d = s.dayoff;
      res.json({d});
    }
...
app.put("/viewcoursecoverage", auth, async (req, res) => {
  try {
    ...
    if (!coursefind) {
      return res
        .json({ msg: "Course not found in your department" });
    }
...
app.put("/viewreplacementbyid", auth, async (req, res) => {
  try {
    ...
    if (!request) {
      return res.json({ msg: " This Replacement request was cancelled by the user" });
    }
    res.json({request});
  } catch (error) {
    //important
...
app.get("/viewslots/:id", auth, async (req, res) => {
  try {
    ...
    const allinfo = await courses
      .findOne({ instructors: idmember , id:req.params.id}, { _id: 0, id: 1, name: 1,slots:1 })
      .populate({
        path: "slots",
        select: {
          _id: 0,
...
app.delete("/deleteassigncourse/:id/:idcourse", auth, async (req, res) => {
  try {
    ...
    if (!match1) {
      return res
        .json({ msg: " you not  (course instructor) to this course " });
    }
...
app.post(
  ...
  async (req, res) => {
    try {
      if (req.tokenrole != "Course Instructor") {
        return res
          .json({ msg: "You are not authorized to to this" });
      }
...
app.put("/rejectReplacemant", auth, async (req, res) => {
  try {
    ...
    let myRequest = await requests.findOne(
      { id: requestId }
    );
