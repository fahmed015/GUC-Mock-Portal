        ...
        if (res.data.msg) {
          ...
          if(res.data.output[1]>0){
            //  console.log("here")
              ...
              this.setState({Missingdaydeduction:res.data.output[1]*(res.data.output[2]/60)})
              //salary=salary
            ...
            if(res.data.output[0]<0){
              ...
              if(this.state.calchrs>0){
                ...
                this.setState({Missinghrsdeduction:(Math.floor(this.state.calchrs/60)*(res.data.output[2]/180))})
                this.setState({Missingminsdeduction:((this.state.calchrs%60)*(res.data.output[2]/10800))})
                console.log(Math.floor(this.state.calchrs/60)+"daydayday"+this.state.Missinghrsdeduction)
                console.log("minminmin"+this.state.Missingminsdeduction)
  
              }
            }
         this.setState({salary: (res.data.output[2]-this.state.Missingdaydeduction-this.state.Missinghrsdeduction-this.state.Missingminsdeduction)})
         Swal.fire("My Salary in"+this.state.month+"/"+this.state.year+" is "+this.state.salary)
    ...
    try {
      await axios({
        ...
        if (res.data.mymissinghrs > 0) {
          ...
          Swal.fire("Extra hrs In " + `${this.state.month}` + '/' + `${this.state.year}` + " are : " + Math.floor(res.data.mymissinghrs / 60) +
            ":" + (res.data.mymissinghrs % 60));

        } else {
          let x = 0;
          x = res.data.mymissinghrs * -1;
          Swal.fire("Missin hrs In " + `${this.state.month}` + '/' + `${this.state.year}` + " are : " + Math.floor(x / 60) +
            ":" + (x % 60));
        }
        //  this.setState({Missingdays:res.data.missingdays})
      ...
      <div>
        ...
        <h3>Selected year: {this.state.year}</h3> */}
          ...
          <Row className="justify-content-md-center">
            ...
            <Col xs lg="2">
              <Button onClick={() => { this.viewAttendence() }}
                type="submit"
                style={{
                  backgroundColor: "#456268",
                  color: "white"
            ...
            <Col xs lg="2">
              <Button onClick={() => { this.viewSalary() }}
                type="submit"
                style={{
                  backgroundColor: "#456268",
                  color: "white"
